...
import ImageRenderer from '@/components/component-renderer/notion-renderer/image-renderer'
...
interface ImageChoiceRendererProp {
  parent: ComponentRendererType
  quiz: ChoiceType
}

const ImageChoiceRenderer: React.FC<ImageChoiceRendererProp> = ({ quiz }) => {
            ...
            {quiz?.options?.map((item: any, index: any) => {
              return (
                <div
                  key={index}
                  className={cn(
                    'flex cursor-pointer flex-col items-center gap-4 rounded-xl border-[2px] border-neutral-300 p-4 text-neutral-800 transition-all duration-200 max-sm:w-full sm:w-[calc((100%-1rem)/2)]',
                ...
                >
                  <div className="w-full overflow-hidden rounded-lg">
                    <ImageRenderer
                      nextComponent={null}
                      prevComponent={null}
                      component={item.option.children[0]}
                      position={0}
                      parent={quiz}
                    />
                  </div>
                  <div className="flex w-full items-center justify-between gap-2">
                    <div className="flex-1 flex-shrink-0 text-base">
                      <TextRenderer
