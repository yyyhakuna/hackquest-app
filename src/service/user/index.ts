import { transformQueryString } from '@/lib/formate'
...
import {
  ...
  type PersonalLinksType,
  type RegisterParamsType,
  type RegisterResponse,
  ThirdPartyAuthType,
  type UserExperienceType,
  type UserHackathonType,
  type UserLearnedCountType,
...
class UserApi {
  ...
  getUserProfileByUsername(username: string) {
    return this.service.get<UserProfileType>(
      `${UserApiType.UserProfile}/${username}`,
    )
  }
  ...
  getConnectInfo() {
    return this.service.get<
      { username: string; thirdPartyName: ConnectType }[]
    >(`/auth`)
  }
  ...
  checkTwitterFollow() {
    return this.service.get<{ isFollow: boolean }>(
      UserApiType.CheckTwitterFollow,
    )
  }

  /** 获取所有nitification */
  getNotifications(params: object) {
    return this.service.get<{ total: number; data: NotificationType[] }>(
      UserApiType.Notifications,
      {
        params,
      },
    )
