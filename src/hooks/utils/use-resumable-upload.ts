...
export const useResumableUpload = (config: UploadConfig) => {
  const { chunkSize = 1024 * 1024 } = config
  const [_currentChunk, _setCurrentChunk] = useState<Buffer>()
  const [nextChunkIndex, setNextChunkIndex] = useState(0)
  const [percent, setPercent] = useState(0)

  const _cacheKey = `${config.cachePrefix}-upload-pending-cache`
  ...
  const resumableUpload = async (
    ...
    const response = await fetch(url || sessionUrl!, {
      ...
      headers: {
        ...
        'Access-Control-Allow-Origin': getDomain('local'),
      } as any,
      body: chunk,
    })
    ...
    if (response.ok) {
      ...
      return webApi.commonApi.getAssetsUrl(
        optionsInstance.current?.sessionUrl.split('?')[0]!,
      )
    }
